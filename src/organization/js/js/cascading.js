...
module.exports = function($, undefined) {
  ...
  Dropdown.prototype = {
    ...
    _requirementsMet: function() {
      ...
      if (self.options.requireAll) {
        // If requireAll is true, return true if all dropdowns have values
        return (
          self.requiredDropdowns.filter(function() {
            return !!$(this).val();
          }).length == self.options.requires.length
        );
      } else {
        // Otherwise, return true if any one of the required dropdowns has value
        return (
          self.requiredDropdowns.filter(function() {
            return !!$(this).val();
          }).length > 0
    ...
    _initSource: function() {
      ...
      if ($.isArray(self.options.source)) {
        this.source = function(request, response) {
          response(
            $.map(self.options.source, function(item) {
              return {
    ...
    setSelected: function(indexOrValue, triggerChange) {
      ...
      if (self.el.is('[multiple]')) {
        selectedValue = selectedItems.map(function(item) {
          if (typeof item === 'number' && (item !== undefined && item > -1 && item < dropdownItems.length)) {
        ...
        if (
          ...
          (selectedItems[0] !== undefined && selectedItems[0] > -1 && selectedItems[0] < dropdownItems.length)
        ) {
  ...
  CascadingDropdown.prototype = {
    _init: function() {
      ...
      var dropdowns = $(
        $.map(self.options.selectBoxes, function(item) {
      ...
      function readyEventHandler(event) {
        if (++counter == dropdowns.length) {
          // Once all dropdowns are ready, unbind the event handler, and execute onReady
